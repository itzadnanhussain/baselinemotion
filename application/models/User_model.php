...
Class User_model extends CI_Model {
	...
	function fetchsigns(){
		...
		if($query->num_rows() > 0) {
          // Got some rows, return as assoc array
			...
			foreach($userobj as $user){
				$data2 = array();
				$data2['name'] = $user->name;
				$data2['sign'] = $user->sign;
				$data[] = $data2;
			}
        }
	...
	function fetchparams($id){
		...
		$this->db->select('SQL_CALC_FOUND_ROWS *', FALSE);
		$this->db->from('workout_param');
		$this->db->where('param_type', $id); 
		$this->db->order_by('workout_param.param_label', 'ASC');
		$query = $this->db->get();
		...
		if($query->num_rows() > 0) {
          // Got some rows, return as assoc array
			...
			foreach($userobj as $user){
				$data2 = array();
				$data2['id'] = $user->id;
				$data2['param_name'] = $user->param_name;
				$data2['param_label'] = $user->param_label;
				$data[] = $data2;
			}
        }
		return $data;
	...
	function fetchcombinescores($id){
		$data = array();
		
		$this->db->select('SQL_CALC_FOUND_ROWS *', FALSE);
		$this->db->from('combine_scores');
		$this->db->where('user_id', $id); 
		$query = $this->db->get();
	...
	function usersfetchrules($patientid)
	{
		...
		$this->db->where('workout_rules.status', 1);
	...
	function fetchcombinescoresembed($id){
		$data = array();
		...
		if($query->num_rows() > 0) {
          // Got some rows, return as assoc array
			...
			foreach($userobj as $user){
				...
				$data2['name'] = $user->name;
				$data2['param_name'] = $user->param_name;
				$data2['param_value'] = $user->param_value;
				$data2['param_sign'] = $user->param_sign;
	...
	function selectWallangelRules() {
		$this->db->select('workout_rules.*,workout_param.param_label');
		$this->db->from('workout_rules');
		$this->db->where('workout_rules.rules_type', 'WallAngel');
		$this->db->join('workout_param', 'workout_param.param_name = workout_rules.param_name','left');		
		$this->db->order_by('workout_rules.id', 'DESC');
		$query = $this->db->get();
		return $query;
	}
	
	function selectOverheadRules() {
		$this->db->select('workout_rules.*,workout_param.param_label');
		$this->db->from('workout_rules');
		$this->db->where('workout_rules.rules_type', 'Overhead');
		$this->db->join('workout_param', 'workout_param.param_name = workout_rules.param_name','left');		
		$this->db->order_by('workout_rules.id', 'DESC');
		$query = $this->db->get();
		return $query;
	...
	function selectKAMSRules() {
		$this->db->select('workout_rules.*,workout_param.param_label');
		$this->db->from('workout_rules');
		$this->db->where('workout_rules.rules_type', 'KAMS'); 
		$this->db->join('workout_param', 'workout_param.param_name = workout_rules.param_name','left');
		$this->db->order_by('workout_rules.id', 'DESC');
		$query = $this->db->get();
		return $query;
	...
	function selectPostureRules() {
		$this->db->select('workout_rules.*,workout_param.param_label');
		$this->db->from('workout_rules');
		$this->db->where('workout_rules.rules_type', 'Posture');
		$this->db->join('workout_param', 'workout_param.param_name = workout_rules.param_name','left');		
		$this->db->order_by('workout_rules.id', 'DESC');
		$query = $this->db->get();
		return $query;
	...
	function selectROMRules() {
		$this->db->select('workout_rules.*,workout_param.param_label');
		$this->db->from('workout_rules');
		$this->db->where('workout_rules.rules_type', 'ROM');
		$this->db->join('workout_param', 'workout_param.param_name = workout_rules.param_name','left');		
		$this->db->order_by('workout_rules.id', 'DESC');
		$query = $this->db->get();
		return $query;
	...
	function selectcombinescores()
	{
		$this->db->select('combine_scores.*,users.first_name,users.last_name');
		$this->db->from('combine_scores');
		$this->db->join('users', 'users.id = combine_scores.user_id','left');
		$this->db->order_by('combine_scores.id', 'DESC');
		$query = $this->db->get();
		return $query;
	}
	...
	function insertcombinescores($data)
	{
		$this->db->insert('combine_scores', $data);
	}
	...
	function insertrules($data)
	{
		$this->db->insert('workout_rules', $data);
	}
	
	function editrules($id, $data) {
        $this->db->where('id', $id);
        $result = $this->db->update('workout_rules', $data);
	...
	function deletecombinescores($id) {
        $this->db->where('id', $id);
        $this->db->delete('combine_scores'); 
    }
	
	function parentuser($paruser) {
        $this->db->select('*', FALSE);
        $this->db->from('admin_users');
        $this->db->where('id', $this->db->escape_str($paruser)); 
