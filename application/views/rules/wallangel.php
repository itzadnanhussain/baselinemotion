...
<div class="content-wrapper">
    ...
    <section class="content">
		<div class="container box">
			<form method="post" id="add_rules_wallangel" enctype="multipart/form-data">
				...
				<input type="hidden" name='rules_type' id="rules_type" value='WallAngel'>
				<?php if($id != ""){ ?>
					<input  type="hidden" name='id' id="rules_id" value='<?php echo $id; ?>'>
					<button type="submit" name="add_rules" class="btn btn-primary" id="add_rules">Update</button>
				<?php }else{ ?>
					<input  type="hidden" name='status' id="status" value='1'>
					<button type="submit" name="add_rules" class="btn btn-primary" id="add_rules">Save</button>
				<?php } ?>
			</form>
			<br />
			<div id="imported_rules_wallangel_data"></div>
		</div>
...
#imported_rules_wallangel_data .switch {
  position: relative;
...
#imported_rules_wallangel_data .switch input {
  opacity: 0;
  width: 0;
  height: 0;
}
...
/* The slider */
#imported_rules_wallangel_data .slider {
  position: absolute;
  cursor: pointer;
  top: 0;
  left: 0;
...
#imported_rules_wallangel_data .slider:before {
  position: absolute;
  content: "";
  height: 26px;
  width: 26px;
...
#imported_rules_wallangel_data input:checked + .slider {
  background-color: #2196F3;
}

#imported_rules_wallangel_data input:focus + .slider {
  box-shadow: 0 0 1px #2196F3;
}

#imported_rules_wallangel_data input:checked + .slider:before {
...
/* Rounded sliders */
#imported_rules_wallangel_data .slider.round {
  border-radius: 34px;
}

#imported_rules_wallangel_data .slider.round:before {
  border-radius: 50%;
}
