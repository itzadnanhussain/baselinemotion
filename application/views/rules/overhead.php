...
<div class="content-wrapper">
    ...
    <section class="content">
		<div class="container box">
			<form method="post" id="add_rules_overhead" enctype="multipart/form-data">
				...
				<input type="hidden" name='rules_type' id="rules_type" value='Overhead'>
				<?php if($id != ""){ ?>
			...
			<br />
			<div id="imported_rules_overhead_data"></div>
		</div>
...
#imported_rules_overhead_data .switch {
  position: relative;
...
#imported_rules_overhead_data .switch input {
  opacity: 0;
  width: 0;
  height: 0;
}
...
/* The slider */
#imported_rules_overhead_data .slider {
  position: absolute;
  cursor: pointer;
  top: 0;
  left: 0;
...
#imported_rules_overhead_data .slider:before {
  position: absolute;
  content: "";
  height: 26px;
  width: 26px;
...
#imported_rules_overhead_data input:checked + .slider {
  background-color: #2196F3;
}

#imported_rules_overhead_data input:focus + .slider {
  box-shadow: 0 0 1px #2196F3;
}

#imported_rules_overhead_data input:checked + .slider:before {
...
/* Rounded sliders */
#imported_rules_overhead_data .slider.round {
  border-radius: 34px;
}

#imported_rules_overhead_data .slider.round:before {
  border-radius: 50%;
}
